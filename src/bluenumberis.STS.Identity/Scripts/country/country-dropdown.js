...
(function ($) {
	...
	function dd(element, settings) {
		...
		var _styles_i = { li: '_msddli_', ddChildMore: 'border shadow', fnone: "fnone" };
		...
		var _construct = function () {
			...
			var index = Number(settings.initialCountryIndex);
			if (!isNaN(index)) {
				_setValue(index);
			}
		};
		...
		var _createWrapper = function () {
			var obj = {
				className: _styles.dd + " ddcommon custom-select"
			};
		...
		var _createFilterBox = function () {
			...
			var sText = _createElement("input", { id: tid, type: 'text', value: '', autocomplete: 'off', className: 'text shadow', style: 'display: none' });
			return sText;
		};
		...
		var _overrideMethods = function () {
			...
			if (settings.initialCountryCode === undefined || settings.initialCountryCode === '') {
				$.get("https://ipinfo.io", function (response) {
					_updateCountryCode(response.country);
				}, "jsonp");
			}
		};

		var _updateCountryCode = function (countryCode) {
			var elements = $("#" + _element + "_child ul li");
			for (var i = 0; i < elements.length; i++) {
				$(elements[i]).removeClass('selected');
			}

			for (var i = 0; i < elements.length; i++) {
				if ($(elements[i])[0].title === countryCode) {
					$(elements[i]).addClass('selected');
					var image = $("#" + _element + "_title").children()[0];
					var span = $("#" + _element + "_title").children()[1];
					var srcImage = $(elements[i]).children()[0];
					var srcSpan = $(elements[i]).children()[1];
					$(image)[0].src = $(srcImage)[0].src;
					$(span).text($(srcSpan).text());
					break;
				}
			}

			_settings.change(countryCode);
		};
		...
		var _updateTitleUI = function (index, byvalue) {
			...
			if (value.image != "" && _settings.showIcon) {
				img = _createElement("img", { src: value.image, width: 40 });
				$("#" + titleid).prepend(img);
				if (value.imagecss != "") {
