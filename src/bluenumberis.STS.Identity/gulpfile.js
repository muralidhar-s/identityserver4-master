...
function processScripts() {
	return gulp
		.src([
			...
			'./Scripts/country/country-dropdown.js'
		])
...
function processCountryDDCss() {
	return gulp
		.src([
			'Styles/country-dd/dd.css',
			'Styles/country-dd/flags.css'
		])
		.pipe(sass())
		.on('error', sass.logError)
		.pipe(concat('countrydd.css'))
		.pipe(gulp.dest(cssFolder));
}

function processCountryDDCssMin() {
	return gulp
		.src([
			'Styles/country-dd/dd.css',
			'Styles/country-dd/flags.css'
		...
		.on('error', sass.logError)
		.pipe(minifyCSS())
		.pipe(concat('countrydd.min.css'))
		.pipe(gulp.dest(cssFolder));
}

var buildStyles = gulp.series(processStyles, processSass, processSassMin, processCustomCss, processCustomCssMin, processCountryDDCss, processCountryDDCssMin);
...
gulp.task('custom', processCustomCss);
gulp.task('custom:min', processCustomCssMin);
gulp.task('countrydd', processCountryDDCss);
gulp.task('countrydd:min', processCountryDDCssMin);
